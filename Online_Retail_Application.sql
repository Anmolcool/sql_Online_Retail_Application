...
create database stud_mang;
use stud_mang;
/*
SQL can also be utilized to accomplish this project. 
The idea here is to create a project that will be accountable for accumulating students’ records. 
Most of these track records need to keep details about the students. 
This information could be the generic details like student’s name, address, performance, attendance, etc, or 
specific information related to departments like a collection of data.

Features needed –

The project’s database should contain general details about the students. 
For instance their address, name, contact information, year of admission, courses, etc.
...
select S.streetaddr, S.starttime , S.endtime from ServicePoint S
inner join Address A
where A.userid = 5 and S.city = A.city;

-- query the information of laptops
select brand, name, type, modelNumber, color, amount, price
from product 
where type = 'laptop';
-- query the total quantity of products from store with storeid 8 in the shopping cart
select sum(quantity) as total_quantity from Save_to_Shopping_Cart;
...
*/
select *from orders;
select orderNumber, paymentState
from orders
where creationTime = '2017-02-12';
 ...
 from comments C
 inner join orders O
 where O.orderNumber = 59203012 and O.creationTime = C.creationTime;
 -- Data Modification
...
from users U
inner join seller S
where U.userid = S.userid and U.name = 'A%';

insert into buyer (userid)
values (1),(4),(9),(10),(15),(30),(32),(61),(63),(88);
...
where paymentState = 'unpaid' 
and year(str_to_date(creationTime,'%Y-%m-%d')) > 2017
and totalAmount > 50; 
...
delete from store
where YEAR(STR_TO_DATE(startTime,'%Y-%m-%d')) < 2017; 
...
select *from view_eid;

-- Update the view
 drop view view_eid;
 
-- we have to drop the view, old version doesn,t support update view
-- update version we can simply use update or replace the view.
select *from view_eid;
create view view_eid as ( 
						select *from product
						where price > (select avg(price) from product)
...
select *from product;
create view product_sal as
							 select * from product P
							 inner join OrderItem O
							 on O.pid = P.pid;
-- The error "duplicate column name 'pid'" occurs because both tables, product and OrderItem, have a column named pid. 
-- To resolve this issue, you should explicitly list out the columns 
-- you want to select from each table and avoid selecting columns with duplicate names.
CREATE VIEW product_sal AS
SELECT P.pid AS product_pid, P.name AS product_name, P.price AS product_price, O.pid AS order_pid
FROM product P
INNER JOIN OrderItem O ON O.pid = P.pid;

select *from product_sal;

-- Update the view
DROP VIEW if exists product_sal;
CREATE VIEW product_sal AS
SELECT P.pid AS product_pid, P.name AS product_name, P.price AS product_price, O.pid AS order_pid
FROM product P
INNER JOIN OrderItem O ON O.pid = P.pid;
...
WHERE TABLE_SCHEMA = 'stud_mang'
  AND TABLE_NAME = 'product';

-- Check whether the products saved to the shopping cart after the year 2017 has quantities of smaller than 10.
select S.quantity, P.name from save_to_shopping_cart S
inner join product P
where S.pid = P.pid and year(STR_TO_DATE(addTime, '%Y-%m-%d')) > 2015
and quantity > 2;
-- Check whether the ordered item has 0 to 10 quantities

show create table save_to_shopping_cart;
/* Following quantity is dedault null */